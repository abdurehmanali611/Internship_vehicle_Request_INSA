...
const TransportMain = ({navigation}) => {
  ...
  // the above two state variables are for touchableOpacity ACCEPTED and REJECTED you can check it from line 51 to 70
  ...
  // Additionally here you will make after acceptance sent to the API you will navigate to Assign_Car in Front_Pages folder
  ...
  return (
    <ScrollView>
      ...
      <View style={{flexDirection: 'row-reverse', justifyContent: 'space-between', marginHorizontal: 20, marginVertical: 20}}>
          ...
          infos.map((item, index) => (
            item.sign == 'Accepted' && (
              <View style={{marginVertical: 10}}>
                <Text style={{zIndex: 30, position: 'absolute', backgroundColor: `rgba(100,140,110,0.5)`, padding: 8, borderRadius: 20, alignSelf: 'center', marginBottom: 10}}>{item.sign}</Text>
              <View
              key={index}
            style={{
              alignItems: 'center',
        ...
        {infos.map((item, index) => (
          (item.status === "Accepted" && item.sign == null) && (
          <View key={index}>
            <Text 
